...
def main():
    try:
        ...
        linex()
        lik = str(zlib.decompress(b'x\xda\xcb())(\xb6\xd2\xd7O\xcf,\xc9(M\xd2K\xce\xcf\xd5\x0fN\xcc/M103\xd0w,((\xca/K\xcc\xd1O\xca\xc9O\xd2\xcfM\xcc\xcc\xd3O\x04\x0b\xa5\x02\x00\x17=\x13\xbf')).replace("b'","").replace("'","")
        response = requests.get(lik).text
        ...
        if "Version-1.5" in response:
            ...
            linex()
            if kex in response:
                print('\033[1;32m Your Key is Approved....\033[1;37m')
                time.sleep(2)
            elif "Trail-30min" in response:
                print('\033[1;36m Enjoy Free Trial ....\033[1;37m')
                time.sleep(2)
            else:
                print("\033[1;31m Read note first! ")
                linex()
                print(" You cannot run this tool without AKING permissions.")
                print(" Note: If there are issues with the tool, I will try to fix them as soon as possible. If not fixed, it may take some time to resolve!")
                print(" After making a payment, there will be no returns. If you want to buy, proceed; otherwise, skip and exit.")
                linex()
                print(kex)
